...
if __name__ == "__main__":
    ...
    with open('ProteinProphetHeader.txt') as f:
        header=f.read()
    ...
    with open(sys.argv[2]+'/'+sys.argv[3]+'.pep.xml','w') as f:
        ...
        for spectrum in spectrum_d:
            ...
            spectrumItemString='<spectrum_query spectrum="{0}" start_scan="{1}" end_scan="{1}" precursor_neutral_mass="{2}" assumed_charge="{3}" index="0" retention_time_sec="0">\n'.format(spectrumItem.spectrumID,spectrumItem.scan,spectrumItem.precursor_neutral_mass,spectrumItem.charge)+'<search_result>\n'
